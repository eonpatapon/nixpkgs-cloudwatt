...
in
{
  ...
  contrailControl = lib.buildImageWithPerps {
    name = "opencontrail/control";
    fromImage = lib.images.kubernetesBaseImage;
    services = [
      {
        name = "opencontrail-control";
        executable = "${contrail32Cw.control}/bin/contrail-control --conf_file /etc/contrail/contrail-control.conf";
        preStartScript = ''
  ...
  gremlinServer = lib.buildImageWithPerps {
    ...
    services = [
      {
        name = "gremlin-server";
        executable = "${gremlinServerStart}/bin/gremlin-server";
